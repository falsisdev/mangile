...
const { data: titles, status } = await useLazyFetch<MangaCard[]>(`${config.public.backend.baseUrl}/api/mangaList`, {
  ...
  transform: (data: any) => {
    ...
    return list.map((title: any) => ({
      ...
      suffix: '(' + title.mal_year + ') | ' + title['mal_type'].replaceAll('MANGA', 'Manga').replaceAll('NOVEL', 'Hafif Roman').replaceAll('ONE_SHOT', 'One-Shot'),
...
const groups = computed(() => [{
  id: 'titles',
  label: searchTerm.value ? `Aranıyor: “${searchTerm.value}”...` : 'Öne Çıkanlar',
  items: titles.value || [],
  ignoreFilter: true
}])
