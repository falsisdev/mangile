...
const { data: serie, status } = await useLazyAsyncData('title-' + route.params.titleID, async () => {
    try {
        return await $fetch(`${config.public.backend.baseUrl}/api/manga/${route.params.titleID}`)
    } catch (e) {
        return await $fetch(`${config.public.backend.baseUrl}/api/lightNovel/${route.params.titleID}`)
    }
}, {
    transform: (title: any) => {
        return {
...
const customComponents = {
  marks: {
    link: ({ value }, { slots }) => h(
      'a', 
      { href: value?.href, class: 'text-primary hover:underline font-semibold', target: '_blank' }, 
      slots.default?.()
    )
  }
}
...
<template>
    <UContainer class="space-y-8 relative">
        <div v-if="status === 'pending'" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div class="md:col-span-3 space-y-4">
                    ...
                    <div class="flex gap-2"><USkeleton class="h-6 w-16" v-for="i in 3" :key="i" /></div>
        ...
        <div v-else-if="serie" class="space-y-6">
            <div class="relative h-64 md:h-96 w-full overflow-hidden rounded-3xl transition-opacity duration-150 ease-out" :style="{ opacity: bannerOpacity }">
                <div class="absolute inset-0 bg-gradient-to-t from-default via-default/40 to-transparent z-10"></div>
                <div class="absolute inset-0 bg-gradient-to-b from-default/20 via-transparent to-transparent z-10"></div>
                <img :src="serie.banner || serie.anilistBanner" class="w-full h-full object-cover select-none" alt="Banner" />
            </div>
                ...
                <div class="flex flex-col items-center md:items-stretch space-y-4">
                    ...
                    <div class="w-full space-y-2 max-w-[240px] md:max-w-none">
                        ...
                        <div class="text-xs space-y-2 pt-2 text-muted-foreground">
                            <p v-if="serie.malStatus"><b class="text-foreground">Durum:</b> {{ serie['malStatus'].replaceAll('Publishing', 'Yayınlanıyor') }}</p>
                            <p v-if="serie.malAuthors?.length"><b class="text-foreground">Yazar:</b> {{ serie.malAuthors.map((a: any) => a.name).join(" ").replaceAll(", ", " ") }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="md:col-span-3 space-y-6">
                    <div class="space-y-2 text-center md:text-left">
                        <div class="flex flex-wrap items-center justify-center md:justify-start gap-2">
                            <UBadge color="primary" variant="solid" size="md">{{ serie.type || 'Manga' }}</UBadge>
                            <UBadge v-for="tag in serie.tags" :key="tag" color="primary" variant="subtle" size="md">
                                {{ tag }}
                            </UBadge>
                        </div>
                        ...
                        <h2 v-if="serie.anilistTitle && serie.anilistTitle !== serie.title" class="text-lg text-muted-foreground font-medium">
                            {{ serie.anilistTitle }}
                        </h2>
                    </div>
                    ...
                    <div v-if="serie.notes && serie.notes.length" class="space-y-2">
                        ...
                        <div class="p-5 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-sm prose dark:prose-invert prose-amber max-w-none">
                    ...
                    <div class="space-y-4 pt-4">
                        <div class="flex items-center justify-between">
                            <h3 class="text-xl font-black flex items-center gap-2">
                                ...
                                <span class="text-sm font-normal text-muted-foreground">({{ serie.chapters?.length || 0 }} Bölüm)</span>
                            </h3>
                        </div>

                        <div v-if="serie.chapters && serie.chapters.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            <UCard 
