...
const customComponents: Record<string, Record<string, unknown>> = {
  marks: {
    link: (props: Record<string, unknown>, context: Record<string, () => unknown>) =>
      h(
        'a',
        {
...
function getChapterNumber(chapter: ChapterData): number {
  const rawValue = chapter.chapter_number ?? (chapter as Record<string, unknown>).number ?? chapter.id
...
function getChapterDisplayValue(chapter: ChapterData | null): string {
  ...
  if (chapter.chapter_number !== undefined && chapter.chapter_number !== null && chapter.chapter_number !== '') {
    return String(chapter.chapter_number)
  }

  const anyCh = chapter as Record<string, unknown>
  if (anyCh.number !== undefined && anyCh.number !== null && anyCh.number !== '') {
...
const allChaptersSortedAsc = computed(() => {
  ...
  return [...serie.value.chapters].sort((a, b) => getChapterNumber(a) - getChapterNumber(b))
})

const firstChapter = computed(() => allChaptersSortedAsc.value[0] || null)
const lastChapter = computed(() => allChaptersSortedAsc.value[allChaptersSortedAsc.value.length - 1] || null)

const sortedChapters = computed(() => {
  const list = chapterSortOrder.value === 'asc'
  ...
  return list.filter((ch) => {
    ...
    const rawNumStr = String(ch.chapter_number ?? (ch as Record<string, unknown>).number ?? ch.id ?? '').toLowerCase()
  ...
  <UContainer class="space-y-8 relative pb-20 md:pb-0">
    ...
    <div v-if="status === 'pending'" class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div class="space-y-4">
          <USkeleton class="h-72 w-full rounded-2xl mx-auto max-w-[240px] md:max-w-none" />
    ...
    <div v-else-if="serie" class="space-y-6">
      ...
      <div class="block md:hidden space-y-6 -mx-4 -mt-4">
        <!-- Mobil Hero Kartı -->
        <div class="relative bg-muted/30 border-b border-border/40 pb-4 overflow-hidden">
          <!-- Arka Plan Görseli ve Koyu Karartma Katmanları -->
          <div class="relative h-56 w-full overflow-hidden">
            <img :src="serie.banner || serie.anilistBanner || serie.cover || serie.anilistCover"
              class="w-full h-full object-cover" alt="Banner" />
            <div class="absolute inset-0 bg-gradient-to-t from-background via-background/80 to-black/70 z-10" />
          </div>
          ...
          <div class="px-4 -mt-24 relative z-20 flex gap-4 items-end">
            ...
            <div class="space-y-1.5 pb-1 overflow-hidden">
              ...
              <h1 class="text-2xl font-black text-foreground leading-tight drop-shadow-md line-clamp-2">
                {{ serie.title }}
              </h1>
              <p v-if="serie.anilistTitle && serie.anilistTitle !== serie.title"
          ...
          <div class="px-4 pt-4 flex flex-wrap gap-2 items-center">
            <div
              ...
              <UIcon name="i-lucide-star" class="w-4 h-4 text-emerald-500 fill-current" />
            ...
            <div
              class="px-3 py-1 rounded-xl bg-muted/80 ring-1 ring-default/30 text-xs font-bold flex items-center gap-1.5">
              <UIcon name="i-lucide-star" class="w-4 h-4 text-amber-500 fill-current" />
        ...
        <!-- Mobil Detaylar ve İçerik -->
        <div class="px-4 space-y-5">
          <!-- Mobil Özet -->
          <div class="space-y-2 bg-muted/20 p-4 rounded-2xl ring-1 ring-default/20">
            <h3 class="text-sm font-bold text-foreground flex items-center gap-2">
              <UIcon name="i-lucide-book-open" class="text-primary w-4 h-4" /> Özet
            </h3>
            <div class="relative">
              <p class="text-xs text-muted-foreground leading-relaxed transition-[max-height] duration-500 ease-in-out overflow-hidden"
            ...
            <button class="text-xs font-bold text-primary flex items-center gap-1 pt-1"
              @click="isDescriptionExpanded = !isDescriptionExpanded">
              {{ isDescriptionExpanded ? 'Daralt' : 'Devamını Oku' }}
              <UIcon :name="isDescriptionExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
                class="w-3.5 h-3.5" />
            </button>
          </div>

          <!-- Mobil Editör Notu -->
          <div v-if="serie.notes && serie.notes.length"
            ...
            <h3 class="text-sm font-bold text-amber-500 flex items-center gap-2">
              <UIcon name="i-lucide-sticky-note" class="w-4 h-4" /> Editör Notu
            </h3>
            <div class="relative">
              <div
                ...
                <PortableText :value="serie.notes" :components="customComponents" />
              </div>
              <div v-if="!isNotesExpanded"
                class="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-background/90 to-transparent pointer-events-none" />
            </div>
            <button class="text-xs font-bold text-amber-500 flex items-center gap-1 pt-1"
              @click="isNotesExpanded = !isNotesExpanded">
              {{ isNotesExpanded ? 'Daralt' : 'Oku' }}
              <UIcon :name="isNotesExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="w-3.5 h-3.5" />
            </button>
          </div>
      ...
      <!-- ================= MASAÜSTÜ GÖRÜNÜM (Desktop Layout) ================= -->
      <div
        ...
        <div class="absolute inset-0 bg-gradient-to-t from-default via-default/40 to-transparent z-10" />
        <div class="absolute inset-0 bg-gradient-to-b from-default/20 via-transparent to-transparent z-10" />
        <img :src="serie.banner || serie.anilistBanner || serie.cover || serie.anilistCover"
          class="w-full h-full object-cover select-none" alt="Banner" />
      </div>

      <!-- Masaüstü Orijinal Grid Yapısı -->
      <div class="hidden md:grid grid-cols-1 md:grid-cols-4 gap-8 relative z-20 -mt-24 md:-mt-24 px-2 md:px-4">
        <!-- Sol Kolon: Kapak & Metrikler (col-span-1) -->
        <div class="flex flex-col items-center md:items-stretch space-y-4">
          ...
          <div class="w-full space-y-2.5 max-w-60 md:max-w-none">
            <UButton v-if="serie.malURL" :to="serie.malURL" target="_blank" color="neutral" variant="subtle" block
              icon="i-lucide-external-link" class="rounded-xl font-medium">
              MyAnimeList'te Gör
            </UButton>

            <div class="grid grid-cols-2 gap-2 text-center pt-1">
              <div class="p-2.5 rounded-xl bg-muted/40 ring-1 ring-default/30 backdrop-blur-xs">
                <span class="text-[11px] text-muted-foreground block font-medium mb-0.5">AniList Skoru</span>
                <div class="flex items-center justify-center gap-1 font-extrabold text-emerald-500 text-sm">
              ...
              <div class="p-2.5 rounded-xl bg-muted/40 ring-1 ring-default/30 backdrop-blur-xs">
                <span class="text-[11px] text-muted-foreground block font-medium mb-0.5">MAL Skoru</span>
                <div class="flex items-center justify-center gap-1 font-extrabold text-amber-500 text-sm">
                  <UIcon name="i-lucide-star" class="w-3.5 h-3.5 fill-current" />
                  {{ serie.malScore || "-" }}
                </div>
              </div>
            ...
            <div
              ...
              <p v-if="serie.malStatus" class="flex justify-between items-center">
              ...
              <p v-if="serie.malAuthors?.length" class="flex justify-between items-center gap-2 truncate">
                <span>Yazar:</span>
        ...
        <!-- Sağ Kolon: Başlık, Özet VE Bölümler (md:col-span-3 İçinde Sabit) -->
        <div class="md:col-span-3 space-y-6">
          <div class="space-y-2 text-center md:text-left">
            <div class="flex flex-wrap items-center justify-center md:justify-start gap-2">
              <UBadge color="primary" variant="solid" size="md" class="rounded-lg font-bold">
                {{ serie.type || "Manga" }}
              </UBadge>
            ...
            <h2 v-if="serie.anilistTitle && serie.anilistTitle !== serie.title"
          ...
          <!-- Özet -->
          <div class="space-y-2">
            <h3 class="text-base font-bold flex items-center gap-2 text-foreground">
              <UIcon name="i-lucide-book-open" class="text-primary w-5 h-5" /> Özet
            </h3>
            <div class="relative">
              <div
            ...
            <div v-if="(serie.description || serie.anilistDescription || '').length > 150" class="flex justify-end">
              <UButton color="neutral" variant="ghost" size="xs"
                :icon="isDescriptionExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="font-semibold"
                @click="isDescriptionExpanded = !isDescriptionExpanded">
          ...
          <div v-if="serie.notes && serie.notes.length" class="space-y-2">
            <h3 class="text-base font-bold text-amber-500 flex items-center gap-2">
              <UIcon name="i-lucide-sticky-note" class="w-5 h-5" /> Editör Notları
            </h3>
            ...
            <div class="flex justify-end">
              <UButton color="warning" variant="ghost" size="xs"
                :icon="isNotesExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="font-semibold"
                @click="isNotesExpanded = !isNotesExpanded">
          ...
          <!-- Masaüstü Bölümler Bölgesi (md:col-span-3 İçinde) -->
          <div id="chapters-section" class="space-y-4 pt-4">
            <!-- Başlık ve Kontroller -->
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-border/40 pb-3">
              <h3 class="text-xl font-black flex items-center gap-2">
                <UIcon name="i-lucide-layers" class="text-primary" /> Bölümler
                <UBadge color="neutral" variant="soft" size="md" class="rounded-md font-bold ">
                  {{ serie.chapters?.length || 0 }} Bölüm
                </UBadge>
              </h3>

              <div v-if="serie.chapters && serie.chapters.length" class="flex flex-wrap items-center gap-2">
                <UInput v-model="chapterSearchQuery" icon="i-lucide-search" placeholder="Bölüm ara..." size="sm"
                  color="neutral" variant="outline" class="w-full sm:w-48 rounded-xl" clearable />
                ...
                <div class="flex items-center gap-0.5 p-1 bg-muted/60 rounded-xl ring-1 ring-default/20">
                  <UButton icon="i-lucide-layout-grid" :variant="chapterViewMode === 'grid' ? 'solid' : 'ghost'"
                    color="primary" size="xs" class="rounded-lg" @click="chapterViewMode = 'grid'" />
                  <UButton icon="i-lucide-layout-list" :variant="chapterViewMode === 'list' ? 'solid' : 'ghost'"
                    color="primary" size="xs" class="rounded-lg" @click="chapterViewMode = 'list'" />
                  <UButton icon="i-lucide-layout-template" :variant="chapterViewMode === 'compact' ? 'solid' : 'ghost'"
                    color="primary" size="xs" class="rounded-lg" @click="chapterViewMode = 'compact'" />
                </div>
              </div>
            </div>

            <!-- Bölüm Kartları Konteynırı (3 Kolon Genişliğinde) -->
            <div v-if="sortedChapters.length" class="space-y-4">
              <div class="relative">
                <div class="transition-[max-height] duration-500 ease-in-out overflow-hidden" :class="[
                  isChaptersExpanded || chapterSearchQuery.trim() ? 'max-h-[25000px]' : 'max-h-[580px]',
                  {
                  ...
                  <UCard v-for="chapter in sortedChapters" :key="chapter.id"
                    ...
                    <NuxtLink :to="`/chapter/${chapter._key}/${chapterRouteType(serie?.type)}`"
                      ...
                      <div class="truncate pr-1">
                        <p class="font-bold text-sm group-hover:text-primary transition-colors truncate">
                          {{ chapter.title || `Bölüm ${chapter.chapter_number || chapter.id}` }}
                        </p>
                        <p v-if="chapterViewMode !== 'compact'"
                      ...
                      <UButton v-if="chapterViewMode !== 'compact'" size="xs" color="primary" variant="ghost"
                        icon="i-lucide-chevron-right" square class="group-hover:translate-x-0.5 transition-transform" />
                    </NuxtLink>
                  </UCard>
                </div>
              ...
              <div v-if="sortedChapters.length > chapterLimit && !chapterSearchQuery.trim()"
                ...
                <UButton :icon="isChaptersExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" variant="soft"
                  color="primary" class="rounded-xl px-6 font-semibold"
            ...
            <div v-else class="text-center py-12 rounded-2xl bg-muted/30 border border-dashed border-default">
              <UIcon name="i-lucide-folder-open" class="w-8 h-8 text-muted-foreground mb-2 mx-auto" />
      ...
      <!-- Ortak Önerilen Seriler -->
      <USeparator position="start" class="font-black text-2xl md:text-3xl mt-12 mb-4 px-4 md:px-0">
