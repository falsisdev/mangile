<template>
  <div class="flex flex-row flex-wrap">
    <span v-if="props.ismanual">
      <span
        v-for="item of JSON.parse(props.tags)"
        :key="item"
        :data-tip="item.tr"
      ...
      >
        <RouterLink
          :to="'/tags/' + item.id"
          class="badge badge-neutral m-1 no-underline"
          >{{ item.tr }}</RouterLink
        >
      </span>
    </span>
    <span v-else>
      <span
        ...
        :key="item"
        :data-tip="`${
          tags[item.id] ? tags[item.id].tr : item.attributes.name.en
        }`"
        class="tooltip tooltip-accent"
      >
        <RouterLink
...
const tags = {
  ...
  "920b825-4181-4a17-beeb-9918b0ff7a30": {
    tr: "Gay",
    id: "920b825-4181-4a17-beeb-9918b0ff7a30",
  },
};
</script>
